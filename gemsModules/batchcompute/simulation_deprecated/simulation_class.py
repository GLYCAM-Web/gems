...
class Simulation:
    ...
    def CreateSubmissionScript(self):
        ...
        self.SQUEUE_NAME =  'MM_' + self.SMAT_ENTRY[SMAT.key_and_values.LABEL_KEY]
        if self.MD_TYPE == SMAT.key_and_values.TYPE_GAS:
            self.NUM_TASKS = '1'
            self.MD_COMMAND='${AMBERHOME}/bin/sander'
            self.MD_DONE_TEXT = conf.MD_Defines.MD_GP_DONE_TEXT #Depending on solvation, this should change.Return later. ## FIXME
            self.MAX_TIME = '00:05:00' #For gas minimization ## FIXME
            gas_mdin_path = '../Min_input_files/CB_GP_Minimization_2017-01-17.mdin' ## FIXME
            copy_mdin = subprocess.Popen(['cp', gas_mdin_path, self.MDIN])
        elif self.MD_TYPE == SMAT.key_and_values.TYPE_SOL:
            self.NUM_TASKS = '2'
            self.MD_COMMAND='mpirun -np ' + self.NUM_TASKS + ' ${AMBERHOME}/bin/pmemd.MPI'
            self.MD_DONE_TEXT = conf.MD_Defines.MD_SOL_DONE_TEXT
            self.MAX_TIME = '01:00:00' #For solvent minimization ## FIXME
            sol_mdin_path = '/website/userdata/OSM_refactor/Min_input_files/CB_Sol_Minimization_2017-01-17.mdin' ## FIXME ## FIXME
            copy_mdin = subprocess.Popen(['cp', sol_mdin_path, self.MDIN])
        self.PARTITION_NAME = 'amber' ## FIXME
        self.NUM_NODES = '1'
        self.AMBERHOME = '/programs/amber16/' ## FIXME
        self.Run_Script_Name = '/' + self.RUN_PREF + '.sh'

        run_script = open(os.path.abspath(os.path.curdir) + self.Run_Script_Name, 'w+') ## FIXME
...
def check_if_dir_content_good(simulation_obj, SMAT_ENTRY):
    ...
    if os.path.isfile(simulation_obj.PARMTOP) == False:
        ...
        print('Parmtop file missing in sub directory %s'%(os.path.abspath(os.path.curdir))) ## FIXME
    if os.path.isfile(simulation_obj.INPCRD) == False:
        input_file_missing = True
        print('Inpcrd file missing in sub directory %s'%(os.path.abspath(os.path.curdir))) ## FIXME
    ...
    if os.path.isfile(simulation_obj.MDOUT) == True:
        ...
        print('MDOUT file already exists in sub directory %s'%(os.path.abspath(os.path.curdir))) ## FIXME
    if os.path.isfile(simulation_obj.MDCRD) == True:
        out_files_exist = True
        print('MDCRD file already exists in sub directory %s'%(os.path.abspath(os.path.curdir))) ## FIXME
    if os.path.isfile(simulation_obj.MDINFO) == True:
        out_files_exist = True
        print('MDINFO file already exists in sub directory %s'%(os.path.abspath(os.path.curdir))) ## FIXME
    if os.path.isfile(simulation_obj.MDLOG) == True:
        out_files_exist = True
        print('MDLOG file already exists in sub directory %s'%(os.path.abspath(os.path.curdir))) ## FIXME
    if os.path.isfile(simulation_obj.MDRST) == True:
        out_files_exist = True
        print('MDRST file already exists in sub directory %s'%(os.path.abspath(os.path.curdir))) ## FIXME
