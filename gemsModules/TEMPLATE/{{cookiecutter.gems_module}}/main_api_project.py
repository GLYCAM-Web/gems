...
class {{cookiecutter.gems_module}}_Project(Project):
    ...
    def __init__(self, **data : Any):
        ...
        self.project_type = "{{cookiecutter.service_id}}" # This is the same as service_id, and is used in the deprecated code as such
        self.parent_entity = "{{cookiecutter.parent_entity}}" # This is the name of the top directory gemsModule if this is a subModule (initial letter captialized)
                                # Otherwise it is the name of the gemsModule directory for the Entity (initial letter capitalized)
        self.entity_id = "{{cookiecutter.gems_module}}" # Do not include the parent directory here - lowercase
        self.service_id = "{{cookiecutter.service_id}}" # This becomes the subdirectory output paths (like cb, gp, ad, gm, pdb, etc)
