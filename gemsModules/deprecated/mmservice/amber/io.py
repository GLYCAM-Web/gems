...
class amberProject(BaseModel):
    ...
    comment: str = Field(None, title="Comment", description="Free-form comment string.")
    ...
    def initialize(self):
        ...
        if self.thisProjectJson is None:
            self.thisProjectJson = (
                gemshome + "/UserSpace/amberJob/gemsAmberProject.json"
            )
        ...
        for line in file:
            if re.search(pattern1, line):
                self.protocolRequiredInputFiles = (
                    line.strip(pattern1).rstrip().split(" ")
                )
        ...
        for infile in self.protocolRequiredInputFiles:
            if not Path(self.localWorkingDirectory + "/" + infile).is_file():
                raise FileNotFoundError(
                    "Required Input File:  " + self.localWorkingDirectory + "/" + infile
                )

        # Check that the control script is present in the protocolDirectory or the full path
        if not Path(
            self.protocolDirectory + "/" + self.protocolControlScriptName
        ).is_file():
            raise FileNotFoundError(
                "Control script:  "
                + self.protocolDirectory
                + "/"
                + self.protocolControlScriptName
            )
        ...
        if self.submissionName is None:
            ...
            if len(self.jobID) > lmax2:
                ...
                len2 = len(self.jobID)
            self.submissionName = (
                self.molecularSystemType[0:len1] + "-" + self.jobID[0:len2]
            )
        ...
        if self.simulationControlScriptPath is None:
            self.simulationControlScriptPath = (
                self.simulationWorkingDirectory + "/" + self.protocolControlScriptName
            )
        ...
        with open(self.thisProjectLog, "a") as f:
            f.write(
                "The JSON object file has been written to the localWorkingDirectory\n"
            )
    ...
    def copy_protocol_files(self):
        try:
            shutil.copytree(
                self.protocolDirectory,
                self.localWorkingDirectory,
                dirs_exist_ok=True,
                copy_function=shutil.copy,
            )
        except OSError as e:
            log.error(
                f"Unable to copy files from protocolDirectory to localWorkingDirectory. Error: {e}\n{traceback.format_exc()}"
            )
            raise e
        try:
            ...
            raise e
        file.write(
            "The project files have been copied from the protocolDirectory to the localWorkingDirectory\n"
        )
        file.close()
...
def troubleshoot():
    ...
    print(
        "The molecular modeling job type is:  " + amber_project.molecularModelingJobType
    )
