...
class Project(BaseModel):
    ...
    def setFilesystemPath(self, specifiedPath: str = None, noClobber: bool = True):
        ...
        if noClobber is True:
            if self.filesystem_path is not None and self.filesystem_path != "":
                message = (
                    "Filesystem Output Path already exists in Project and cannot be clobbered.  It is:\n"
        ...
        try:
            ...
            self.filesystem_path = (
                gemsModules.deprecated.project.settings.default_filesystem_output_path
            )
            return
        # Assign the path based on the return values from common
        if source == "Environment":
            message = (
                "GEMS Filesystem Output Path was set using an environment variable to: \n"
                + str(path)
            )
            log.debug(message)
            ...
            if context == "website":
                if specifiedPath is not None:
                    log.debug(
                        "An output path is specified in website context.  Checking to see if it is allowed."
                    )
                    if (
                        specifiedPath
                        not in gemsModules.deprecated.project.settings.allowed_website_filesystem_paths
                    ):
    ...
    def setServiceDir(self, specifiedDirectory: str = None, noClobber: bool = False):
        ...
        if self.parent_entity == "" or self.parent_entity is None:
            ...
            self.generateCommonParserNotice(
                noticeBrief="GemsError", additionalInfo={"hint": message}
            )
            return
        if self.requested_service == "" or self.requested_service is None:
            message = (
                "Cannot initialize a project without a requested_service specified."
            )
            log.error(message)
            self.generateCommonParserNotice(
                noticeBrief="GemsError", additionalInfo={"hint": message}
        ...
        self.service_dir = os.path.join(
            self.filesystem_path, self.entity_id, self.service_id
        )
    ...
    def setVersionsFilePath(self, specifiedPath: str = None, noClobber: bool = False):
        ...
        if self.filesystem_path is None or self.filesystem_path == "":
            message = (
                "Cannot set versions file path because cannot determine filesystem path"
            )
            log.error(message)
            self.generateCommonParserNotice(
                noticeBrief="GemsError", additionalInfo={"hint": message}
    ...
    def loadVersionsFileInfo(self):
        ...
        if self.versions_file_path is None or self.versions_file_path == "":
            log.error(
                "There was a problem setting the versions file path.  Cannot load versions file info"
            )
            return
        from gemsModules.deprecated.project.projectUtilPydantic import (
            getVersionsFileInfo,
        )
    ...
    def setDownloadUrlPath(self):
        ...
        try:
            if self.host_url_base_path == "":
                log.error(
                    "the host url base path is not set so cannot set download url path."
                )
                return
            from gemsModules.deprecated.project import projectUtilPydantic as utils

            self.download_url_path = utils.buildDownloadUrlPath(
                self.host_url_base_path, self.entity_id, self.service_id, self.pUUID
            )
    ...
    def getDownloadUrlPath(self):
        ...
        if self.download_url_path is None or self.download_url_path == "":
            log.error(
                "The downloadUrlPath was unset, and could not be set, so could not be got."
            )
            return
    ...
    def copyUploadedFiles(self):
        ...
        if self.has_input_files != "True":
            log.error(
                "This project says it does NOT have input files, but copyUploadedFiles was called."
            )
            try:
                common.logic.copyPathFileToPath(
                    self.upload_path, self.uploaded_file_name, self.project_dir
                )
            except Exception as error:
...
def generateProjectSchemaForWeb():
    ...
    try:
        ...
        log.error(
            "There was a problem writing the Project schema to file: " + str(error)
        )
