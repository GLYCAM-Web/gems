...
class PDB_File_Resource(Resource):
    ...
    locationType: str = Field(  # Literal['URL', 'File', 'Payload']
        Literal['RCSB_ID','Path','Payload','URL']='Path',
        title="Location Type",
        description="Where the PDB File can be found.",
    )
    resourceFormat: str = Field(
        Literal['PDB-strict','PDB-unknown','PDB-variant']='PDB-unknown',
        title="Resource Format",
        description="Supported formats will vary with each Entity.",
    )
    resourceRole: str = Field(
        Literal['Complex_Input','Glycomimetics_Output','Ligand','Receptor']=...,
        title="Resource Role",
